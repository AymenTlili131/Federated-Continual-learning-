...
echo "=== Overlap 0: 21 curated experiments to run ==="
...
# Fetch all finished WandB run names once (avoids redundant re-runs for
# experiments whose output dirs are missing locally but completed on WandB).
WANDB_DONE=$(mktemp)
...
# Crashed runs (WandB state != finished, no local final_model.pth) are restarted.
run_exp() {
    local idx="$1" loss="$2"
    local dir_suffix
    dir_suffix=$(echo "$loss" | sed 's/+/_/g; s/\*\([0-9]\)/\1x/g; s/\*/x/g')
    local exp_name="tiny_overlap0_${dir_suffix}"
    if [ -f "$EXPS/${exp_name}/checkpoints/final_model.pth" ] || grep -qxF "$exp_name" "$WANDB_DONE"; then
        echo "✓ SKIP [${idx}/21] ${loss}  (already done)"
        return 0
    fi
    echo "--- [${idx}/21] overlap0: ${loss} ---"
    $RUN --single --model-size tiny --overlap 0 --loss "$loss" \

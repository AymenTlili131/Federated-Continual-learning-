...
if __name__ == "__main__":
    ...
    L_activations=["silu"] #"leakyrelu",,"relu","silu","gelu""tanh"]
    ...
    for exp_idx in range(252):
        split_mnist = SplitMNIST(n_experiences=2, fixed_class_order=ListExperiences[exp_idx],shuffle=False)
        skf = StratifiedKFold(n_splits=20,random_state=1234,shuffle=True)
        for activ in L_activations:
            for init in L_inits :
                print(ListExperiences[exp_idx])
                patience=3
                Margin=0.05
                L_train_acc=[]
                L_test_acc_0=[]
                ...
                test_dataloader_custom0 = DataLoader(dataset=test0_IF, batch_size=60, num_workers=0, shuffle=True)# don't usually need to shuffle testing data# use custom created test Dataset
